...
@section('content')
    ...
    <div class="max-w-3xl mx-auto mb-6">
        <form action="/hall" method="GET" class="flex items-center bg-white shadow-md rounded-lg overflow-hidden">
            <input type="text" class="w-full px-4 py-2 text-gray-700 focus:outline-none" placeholder="Cari buku..."
            ...
            @endif
            <button type="submit" class="bg-blue-500 text-white px-4 py-3 hover:bg-blue-600">
        ...
        <div class="max-w-4xl mx-auto mb-18">
            ...
            <div class="text-center mt-4">
                <h3 class="text-2xl font-bold">
                    <a href="/hall/book/{{ $books[0]->slug }}"
                        class="text-gray-900 hover:text-blue-500 capitalize hover:underline transition">
                        {{ $books[0]->name }}
                    </a>
                </h3>
                ...
                <div class="flex justify-center items-center text-gray-600 text-sm gap-4 mt-2">
                    <span class="flex items-center gap-1">
                        ...
                        <a href="/hall?author={{ $books[0]->author->slug }}"
                            class="hover:text-blue-500">{{ $books[0]->author->name }}</a>
                    </span>
                    <span class="flex items-center gap-1">
                        <i class="fa-solid fa-bookmark text-green-500"></i>
                        <a href="/hall?category={{ $books[0]->category->slug }}"
                            class="hover:text-blue-500">{{ $books[0]->category->name }}</a>
                    </span>
                    <span class="flex items-center gap-1">
                ...
                <p class="text-gray-700 mt-2">
                    {{ Str::limit($books[0]->description, 150) }}
                </p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                ...
                @foreach ($books->skip(1) as $book)
                    <div class="bg-white shadow-md rounded-lg overflow-hidden">
                        <div class="relative">
                            @if ($book->image)
                                <img src="{{ Storage::url($book->cover) }}" class="w-full h-60 object-cover"
                                    alt="Book Cover">
                            @else
                                <img src="https://picsum.photos/400/400?random=1" alt="Book Cover">
                            @endif
                            <div class="absolute top-2 left-2 bg-black/70 text-white px-3 py-1 text-xs rounded">
                                <a href="/hall?category={{ $book->category->slug }}"
                                    class="hover:underline">{{ $book->category->name }}</a>
                            </div>
                        </div>
                        <div class="p-4">
                            <h5 class="text-lg font-bold">
                                <a href="/hall/book/{{ $book->slug }}"
                                    class="hover:text-blue-500 capitalize hover:underline transition">{{ $book->name }}</a>
                            </h5>
                            <div class="flex items-center text-gray-600 text-sm gap-4 mt-2">
                                <span class="flex items-center gap-1">
                                    <i class="fa-solid fa-user text-blue-600"></i>
                                    <a href="/hall?author={{ $book->author->slug }}"
                                        class="hover:text-blue-500">{{ $book->author->name }}</a>
                                </span>
                                <span class="flex items-center gap-1">
                                    <i class="fa-solid fa-clock text-yellow-300"></i>
    ...
    @else
        <p class="text-center text-gray-600 mt-10">Buku tidak ditemukan.</p>
    @endif
