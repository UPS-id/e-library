...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-4">
        <div class="col-span-12 lg:col-span-full p-4">
                    ...
                    <tbody>
                        @forelse ($users as $user)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4">{{ $user->slug }}</td>
                                <td class="px-6 py-4">{{ $user->email }}</td>
                                <td class="px-6 py-4">{{ $user->role }}</td>
                                <td class="px-6 py-4 flex gap-2">
                                    <a href="{{ route('user.edit', $user->slug) }}" class="text-yellow-500 hover:underline">
                                        <i class="fas fa-pen-to-square"></i> Edit
                                    </a>
                                    <span>|</span>
                                    <form action="{{ route('user.destroy', $user->slug) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
